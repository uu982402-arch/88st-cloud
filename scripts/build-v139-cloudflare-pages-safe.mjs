...
for(const f of baseChecks){
  if(!fs.existsSync(f)){ console.error(`[V139 BUILD SAFE FAIL] V138-6 base artifact missing: ${f}`); process.exit(1); }
}
...
fs.writeFileSync('reports/v139-cloudflare-build-safe-report.json', JSON.stringify({ok:true,version:'V139_BLOG_CONTENT_DIFFERENTIATION_SEO_INTENT',base:'V138-6 artifacts present',mode:'cloudflare-pages-safe-build',generatedAt:new Date().toISOString()}, null, 2));
