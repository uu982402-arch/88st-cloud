...
import {
  ROOT, loadPosts, listIndexFiles, routeFromFile, isPublicRoute,
  extractTagText, extractMetaContent, writeText
} from './lib/site-automation.mjs';

const posts = await loadPosts();
const postMap = new Map(posts.map((post) => [post.path.endsWith('/') ? post.path : `${post.path}/`, post]));
...
for (const file of files) {
  ...
  if (/noindex/i.test(robots)) issue.push('공개 페이지 noindex');
...
await writeText(path.join(ROOT, 'docs', 'indexing-audit-20260323.md'), markdown);
