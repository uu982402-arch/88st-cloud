...
const VERSION = 'static-v72-1-blog-lock-20260524';
...
<head>
  ...
  <link rel="stylesheet" href="/assets/css/v72-1.blog-lock.css?v=${VERSION}" data-v72-blog="true">
</head>
<body class="v71-main-home v72-blog-hub v72-blog-lock" data-v72-1-blog-lock="true">
  <!-- V72_1_BLOG_LOCK_ACTIVE -->
...
${pagination.join('\n')}
  ...
  <script>window.__V72_1_BLOG_POSTS__ = ${JSON.stringify(posts)};</script>
  <script src="/assets/js/v72-1.blog-lock.js?v=${VERSION}" defer data-v72-blog="true"></script>
</body>
</html>
`;
...
packageJson.scripts.verify = 'node scripts/verify-v72-1-blog-lock.mjs';
...
packageJson.scripts['quality:v72-1'] = 'node scripts/generate-v72-1-blog-lock.mjs';
packageJson.scripts['verify:v72-1'] = 'node scripts/verify-v72-1-blog-lock.mjs';
...
if (!packageJson.scripts.build.includes('node scripts/generate-v72-1-blog-lock.mjs')) {
  packageJson.scripts.build = packageJson.scripts.build.replace(' && node scripts/gen-build-ver.mjs', ' && node scripts/generate-v72-1-blog-lock.mjs && node scripts/gen-build-ver.mjs');
}
fs.writeFileSync(packagePath, `${JSON.stringify(packageJson, null, 2)}\n`, 'utf8');

fs.writeFileSync(path.join(root, 'V72_1_BLOG_LOCK_REPORT.md'), `# V72-1 Blog Lock Hotfix Report\n\n- 기준: V71 메인 디자인 시스템 유지\n- 대상: /blog/\n- 상단 대형 타이틀 섹션: 제거\n- 게시글 총량: ${posts.length}\n- 1페이지 노출: 50개\n- 페이지 수: ${totalPages}\n- PC 레이아웃: 3~4열 글래스 카드 그리드\n- 모바일 레이아웃: 1열 콤팩트 글래스 바\n- 정렬 방식: 실제 analytics 데이터가 ZIP 내부에 없으므로 SEO/검색 의도/키워드 기반 안정 점수로 인기순 대체\n- 삭제 파일: 0개\n`, 'utf8');
console.log(`[V72-1] blog lock generated. posts=${posts.length} firstPage=${firstPage.length} pages=${totalPages}`);
