...
function removeOld(txt) {
  ...
  return txt
    ...
    .replace(/\n?<style\b(?=[^>]*id=["']v47-seo-prepass-guard["'])[^>]*>[\s\S]*?<\/style>\s*/ig, "\n")
...
function blogVisualGuard() {
  return `<style id="v47-seo-prepass-guard">
...
for (const page of pages) {
  ...
  if (page.kind === "blog_article" || (page.rel.startsWith("blog/") && /pro-blog-page/i.test(txt))) {
    ...
    if (!/v47-seo-prepass-guard/i.test(txt)) txt = txt.replace("</head>", blogVisualGuard() + "\n</head>");
  }
