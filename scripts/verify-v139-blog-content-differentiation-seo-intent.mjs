...
const errors=[];
const required=['assets/css/v139-blog-content-differentiation.css','scripts/build-v139-cloudflare-pages-safe.mjs','scripts/generate-v139-blog-content-differentiation-seo-intent.mjs','scripts/verify-v139-blog-content-differentiation-seo-intent.mjs','V139_PATCH_MANIFEST.json','V139_UPGRADE_REPORT.md','reports/v139-blog-content-differentiation-audit.json'];
for(const f of required) if(!fs.existsSync(p(f))) errors.push('missing '+f);
const pkg=JSON.parse(read(p('package.json')));
if(pkg.scripts?.build !== 'node scripts/build-v139-cloudflare-pages-safe.mjs') errors.push('package build not V139');
if(pkg.scripts?.verify !== 'node scripts/verify-v139-blog-content-differentiation-seo-intent.mjs') errors.push('package verify not V139');
...
if(errors.length){console.error('[V139 VERIFY FAIL]'); for(const e of errors) console.error('- '+e); process.exit(1)}
fs.mkdirSync(p('reports'),{recursive:true});
fs.writeFileSync(p('reports/v139-verify-report.json'), JSON.stringify({ok:true,version:VERSION,blogHtml:blogCount,v139Css,diffNotes,genericH2,numbering9,metaOk,ga4Ok,navShort,generatedAt:new Date().toISOString()},null,2));
console.log('[V139 VERIFY PASS] blog content differentiation + SEO intent OK', JSON.stringify({blogHtml:blogCount,diffNotes,metaOk,ga4Ok},null,2));
