...
const themeCss = pickLatestByVersion(cssFiles, /^theme\.tokens\.v(\d+).*\.css$/i);
const typoCss = pickLatestByVersion(cssFiles, /^typography\.premium\.v(\d+).*\.css$/i);
...
const globalUnifyCss = pickLatestByVersion(cssFiles, /^global\.unify\.v(\d+).*\.css$/i);

// PRO suite (auto)
const proSuiteCss = pickLatestByVersion(cssFiles, /^pro-suite\.v(\d+)\.css$/i);

const uiFixJs = pickLatestByVersion(jsFiles, /^ui\.fix\.v(\d+).*\.js$/i);
const uiGuardJs = pickLatestByVersion(jsFiles, /^ui\.guard\.v(\d+).*\.js$/i);
const siteRuntimeJs = pickLatestByVersion(jsFiles, /^site\.runtime\.v(\d+).*\.js$/i);
const shareSessionJs = pickLatestByVersion(jsFiles, /^share\.session\.v(\d+).*\.js$/i);
...
const globalUnifyJs = pickLatestByVersion(jsFiles, /^global\.unify\.v(\d+).*\.js$/i);

// PRO suite (auto)
const proSuiteJs = pickLatestByVersion(jsFiles, /^pro-suite\.v(\d+)\.js$/i);

const buildAssets = {
  css: [themeCss, typoCss, luxeCss, hubCss, patchCss, globalUnifyCss, proSuiteCss].filter(Boolean).map((f) => `/assets/css/${f}`),
  js: [uiFixJs, uiGuardJs, siteRuntimeJs, shareSessionJs, globalJs, searchTypingJs, globalUnifyJs, proSuiteJs].filter(Boolean).map((f) => `/assets/js/${f}`),
};
