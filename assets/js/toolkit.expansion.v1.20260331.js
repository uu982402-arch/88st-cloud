...
(() => {
  ...
  function initAddressTracker(){
    ...
    form.addEventListener('submit',(e)=>{ e.preventDefault(); const raw=String(input.value||'').trim(); if(!raw) return empty(result, '입력값이 비어 있습니다.', '사이트명이나 도메인을 넣어 주세요.');
      ...
      const nextLinks = `<div class="lookup-links">${[
        {label:'구글링 페이지', href:`/muktu-police/search/?q=${encodeURIComponent(raw)}&type=${encodeURIComponent('주소')}`, text:'검색 조합을 더 넓게 확인합니다.'},
        {label:'공식주소 체크', href:`/tools/official-check/${domain?`?domain=${encodeURIComponent(domain)}`:''}`, text:'마지막 체크리스트로 넘어갑니다.'},
        {label:'검색어 팩', href:`/tools/search-pack/${raw?`?q=${encodeURIComponent(raw)}`:''}`, text:'관련 검색 조합을 더 많이 만듭니다.'}
      ].map((item)=>`<article class="lookup-link-card"><a href="${item.href}">${item.label}</a><p>${item.text}</p></article>`).join('')}</div>`;
      const html = section('주소 변경 검색 조합', '새주소, 리뉴얼, 공지, 텔레그램 흔적을 한 번에 만듭니다.', listCard(queries.map((item)=>({ title:item.title, detail:item.q, actions:`<div class="toolkit-actions"><a class="safety-link-btn ghost" href="${googleUrl(item.q)}" target="_blank" rel="noopener noreferrer">열기</a><button class="safety-copy-btn mint" type="button" data-inline-copy="${esc(item.q)}">복사</button></div>` })))) + section('다음 단계', '검색 후 기술 점검과 공식주소 체크로 이어갑니다.', nextLinks);
  ...
  function initSimilarDomain(){
    ...
    form.addEventListener('submit',(e)=>{ e.preventDefault(); const raw=String(input.value||'').trim(); if(!raw) return empty(result, '입력값이 비어 있습니다.', '브랜드명이나 도메인을 입력해 주세요.');
      ...
      const html = section('유사 도메인 후보', '숫자, 하이픈, 접미어 추가 패턴을 기준으로 정렬했습니다.', table) + section('활용 방법', '기준 주소와 닮은 주소를 먼저 찾고, 점검 후 마지막에 공식주소를 확인합니다.', `<div class="toolkit-note">${esc(raw)} 기준으로 가장 먼저 봐야 할 것은 숫자 추가, bet/vip 접미어, 하이픈 변형입니다. 점검 버튼으로 생성일과 ASN까지 이어서 확인해 두면 좋습니다.</div>`);
      setResult(result, html, 'toolkit-result-stack');
    });
    const q = new URL(location.href).searchParams.get('q'); if(q){ input.value=q; form.dispatchEvent(new Event('submit', {bubbles:true, cancelable:true})); }
  }
  ...
  function initEvidenceBundle(){
    ...
    form.addEventListener('submit', async (e)=>{ e.preventDefault(); const url=String(input.value||'').trim(); if(!/^https?:\/\//i.test(url)) return empty(result, 'URL 형식을 확인해 주세요.', '예: https://example.com/post');
      empty(result, '근거를 추출 중입니다.', '공개 글의 제목, 날짜, 도메인·IP 흔적을 정리하고 있습니다.');
      try {
        ...
        const follow = domainsFromEvidence(ev).map((domain)=>`<article class="lookup-link-card"><a href="/tools/official-check/?domain=${encodeURIComponent(domain)}">${esc(domain)} 점검 ↗</a><p>도메인 생성일, DNS, ASN과 리스크 점수를 같이 확인합니다.</p></article>`).join('');
        const html = section(src.title || '공개 근거 묶음', '원문 링크와 핵심 흔적만 추려서 정리했습니다.', `<div class="score-grid"><div class="score-metric"><span>호스트</span><strong>${esc(src.host||'-')}</strong></div><div class="score-metric"><span>게시일</span><strong>${esc(formatDate(src.publishedAt))}</strong></div><div class="score-metric"><span>도메인</span><strong>${esc((ev.domains||[]).length)}</strong></div><div class="score-metric"><span>IP</span><strong>${esc((ev.ips||[]).length)}</strong></div></div>`) + section('기록용 메모', '제보나 검토 기록에 바로 붙일 수 있는 텍스트입니다.', `<div class="toolkit-copy-box"><pre>${esc(memo)}</pre><div class="toolkit-actions"><button class="safety-copy-btn mint" type="button" data-inline-copy="${esc(memo)}">메모 복사</button><a class="safety-link-btn ghost" href="${esc(src.url||url)}" target="_blank" rel="noopener noreferrer">원문 열기</a></div></div>`) + (follow ? section('연결 점검', '추출된 도메인은 공식주소 체크로 이어서 봅니다.', `<div class="lookup-links">${follow}</div>`) : '');
        setResult(result, html, 'toolkit-result-stack');
  ...
  function renderClusterCards(payload, type){
    ...
    return section('도메인 군집 요약', '네임서버, ASN, /24 대역 기준으로 묶음을 봅니다.', `<div class="score-grid"><div class="score-metric"><span>도메인</span><strong>${esc(payload.domain)}</strong></div><div class="score-metric"><span>생성일</span><strong>${esc(formatDate(rdap.createdAt))}</strong></div><div class="score-metric"><span>네임서버</span><strong>${esc((payload.dns?.nameServers||[]).length)}</strong></div><div class="score-metric"><span>네트워크</span><strong>${esc(nets.length)}</strong></div></div>`) + section('공유 신호', '동일 ASN, 조직, 네임서버를 기준으로 확인합니다.', `<div class="lookup-links">${nets.map((n)=>`<article class="lookup-link-card"><a href="#">${esc(n.ip||'-')}</a><p>${esc([n.asn, n.org, n.country].filter(Boolean).join(' · ') || '-')}</p></article>`).join('') || '<div class="toolkit-note">네트워크 응답이 없습니다.</div>'}</div>`) + section('최근 조회와 겹치는 항목', '같은 네임서버나 /24 대역이 최근 기록과 겹치는지 봅니다.', history.length ? listCard(history.map((hit)=>({title:hit.item.label || hit.item.key, detail:[hit.overlapNs.length?`NS ${hit.overlapNs.join(', ')}`:'', hit.overlapAsn.length?`ASN ${hit.overlapAsn.join(', ')}`:'', hit.overlapSub.length?`대역 ${hit.overlapSub.join(', ')}`:''].filter(Boolean).join(' · ')}))) : `<div class="toolkit-note">최근 조회 이력과 겹치는 군집 힌트가 없습니다.</div>`);
  }
  ...
  function initClusterTool(){
    ...
    form.addEventListener('submit', async (e)=>{ e.preventDefault(); const raw=String(input.value||'').trim(); if(!raw) return empty(result,'입력값이 비어 있습니다.','도메인이나 IP를 입력해 주세요.');
      empty(result, '군집 정보를 불러오는 중입니다.', 'ASN, 네임서버, /24 대역과 최근 조회 이력을 비교합니다.');
      try {
  ...
  function initOfficialCheck(){
    ...
    form.addEventListener('submit', async (e)=>{ e.preventDefault(); const domain=normalizeDomain(input.value); if(!domain || !domain.includes('.')) return empty(result,'도메인 형식을 확인해 주세요.','예: example.com');
      empty(result, '공식주소 체크를 준비 중입니다.', '도메인 생성일, 만료일, DNS, ASN, 리스크 점수를 읽고 있습니다.');
      try {
        ...
        const html = section('공식주소 체크리스트', '점검 전에 보는 항목을 PASS / CHECK 기준으로 정리했습니다.', listCard(items.map((item)=>({title:`${item.ok ? 'PASS' : 'CHECK'} · ${item.title}`, detail:item.detail})))) + section('종합 메모', '점수와 해석을 함께 보며 마지막 결정을 돕습니다.', `<div class="toolkit-note">${esc(risk.commentary || '도메인 생성일, 만료일, DNS, ASN, 공개 검색 흔적을 함께 보세요.')}</div><div class="toolkit-actions"><a class="safety-link-btn ghost" href="/muktu-police/check/?domain=${encodeURIComponent(domain)}">전체 기술 조회</a><a class="safety-link-btn ghost" href="${googleUrl(`${domain} 먹튀`)}" target="_blank" rel="noopener noreferrer">공개 검색</a></div>`);
        setResult(result, html, 'toolkit-result-stack');
  ...
  function initReportTemplate(){
    ...
    form.addEventListener('submit',(e)=>{ e.preventDefault(); const values = {
      ...
      const html = section('제보 텍스트', '운영자에게 전달하거나 별도 기록에 붙이기 쉬운 형식입니다.', `<div class="toolkit-copy-box"><pre>${esc(text)}</pre><div class="toolkit-actions"><button class="safety-copy-btn mint" type="button" data-inline-copy="${esc(text)}">텍스트 복사</button><a class="safety-link-btn ghost" href="/muktu-police/review/">검토 기준 보기</a></div></div>`);
      setResult(result, html, 'toolkit-result-stack');
    });
  }
  ...
  function initRiskCompare(){
    ...
    form.addEventListener('submit', async (e)=>{ e.preventDefault(); const l=normalizeDomain(left.value); const r=normalizeDomain(right.value); if(!l||!r) return empty(result,'도메인 형식을 확인해 주세요.','비교할 두 도메인을 입력해 주세요.');
      empty(result, '비교 정보를 불러오는 중입니다.', '생성일, DNS, ASN, 리스크 점수를 같이 계산합니다.');
      try {
        ...
        const note = (Number(lp.risk?.score||0) === Number(rp.risk?.score||0)) ? '두 도메인의 점수가 비슷합니다. 생성일과 ASN 차이를 더 보세요.' : (Number(lp.risk?.score||0) < Number(rp.risk?.score||0) ? `${l} 쪽이 더 안정적으로 보입니다.` : `${r} 쪽이 더 안정적으로 보입니다.`);
        const html = section('비교 표', '핵심 항목을 한 화면에 정리했습니다.', table) + section('종합 메모', '점수 차이만 보지 말고 생성일, 네임서버, ASN도 같이 봐야 합니다.', `<div class="toolkit-note">${esc(note)}</div><div class="toolkit-actions"><a class="safety-link-btn ghost" href="/tools/official-check/?domain=${encodeURIComponent(l)}">${esc(l)} 체크</a><a class="safety-link-btn ghost" href="/tools/official-check/?domain=${encodeURIComponent(r)}">${esc(r)} 체크</a></div>`);
        setResult(result, html, 'toolkit-result-stack');
  ...
  function initToolsHubShortcuts(){
    ...
    const tools = [
      ['주소 변경 추적기','ADDRESS','새주소 · 리뉴얼 · 공지 · 텔레그램 흔적','/tools/address-tracker/'],
      ['유사 도메인 감지기','SIMILAR','숫자 · 하이픈 · bet/vip 변형 후보','/tools/similar-domain/'],
      ['검색어 팩 생성기','SEARCH PACK','먹튀 · 후기 · 주소 · 도메인 변경 조합','/tools/search-pack/'],
      ['공개 근거 묶음 생성기','EVIDENCE','제목 · 날짜 · 도메인 · IP 메타 정리','/tools/evidence-bundle/'],
      ['IP · ASN 군집 보기','CLUSTER','ASN · /24 대역 · 네임서버 겹침 확인','/tools/ip-asn-cluster/'],
      ['공식주소 확인 체크','OFFICIAL','생성일 · 만료일 · DNS · 점수 체크리스트','/tools/official-check/'],
      ['제보 정리 템플릿','REPORT','검토용 제보 텍스트 자동 작성','/tools/report-template/'],
      ['리스크 스코어 비교기','COMPARE','도메인 두 개의 점수와 생성일 비교','/tools/risk-compare/']
    ];
  ...
  function initMainShortcuts(){
    ...
    if(grid){
      grid.innerHTML = [
        ['구글링','검색 조합 실행','사이트명 · 도메인','/muktu-police/search/',''],
        ['주소 변경','변경 흔적 추적','새주소 · 리뉴얼 · 공지','/tools/address-tracker/','mint'],
        ['유사 도메인','사칭 후보 확인','숫자 · 하이픈 · 변형','/tools/similar-domain/',''],
        ['도메인조회','도메인·IP 점검','등록일 · DNS · ASN','/muktu-police/check/','gold'],
    ...
    if(toolGrid){
      toolGrid.innerHTML = [
        ['TRACKER','주소 변경 추적','새주소 · 리뉴얼 · 공지','/tools/address-tracker/'],
        ['SEARCH PACK','검색어 팩','먹튀 · 후기 · 주소 조합','/tools/search-pack/'],
        ['OFFICIAL','공식주소 체크','생성일 · DNS · 점수 확인','/tools/official-check/']
