(() => {
  ...
  const OFFERS = [
    {
      ...
      title: '안전한토토사이트 ” 양심 “',
      kicker: '양심 회원 전용 이벤트',
      bullets: ['스포츠/슬롯 첫충/매충 10%', '카지노·미겜 첫충 및 매 충전 5%', '출첵 · 룰렛 · 이사 지원금 이벤트', '콤프 최대 카지노 1.2% · 슬롯 4%'],
      code: 'KAKA',
      theme: 'amber',
      href: 'https://평생양심.com/'
    },
  ...
  function formatDate(value) {
    ...
    return new Intl.DateTimeFormat('ko-KR', { dateStyle: 'medium', timeStyle: 'short' }).format(dt);
  }
  ...
  function renderPromoGrid() {
    const grid = document.querySelector('[data-safety-promos]');
    if (!grid) return;
    grid.innerHTML = OFFERS.map((offer) => `
      <article class="promo-card" data-theme="${escapeHtml(offer.theme)}">
        ...
        <div class="promo-code" style="margin-top:16px">
          <div><span>가입코드</span><strong>${escapeHtml(offer.code)}</strong></div>
          <button class="safety-copy-btn mint" type="button" data-copy-code="${escapeHtml(offer.code)}">코드 복사</button>
        </div>
        <div class="promo-actions" style="margin-top:14px">
          <a class="safety-link-btn" href="${escapeHtml(offer.href)}" target="_blank" rel="noopener noreferrer">공식 주소 바로가기</a>
          <a class="safety-link-btn ghost" href="/muktu-police/">안전센터 보기</a>
        </div>
      </article>
    `).join('');
  }
