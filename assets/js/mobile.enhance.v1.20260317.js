(() => {
  ...
  function cleanupInjectedPromos() {
    const selectors = [
      '.auto-promo-zone',
      '#autoPromoStyles',
      '[data-guaranteed-grid]:not(body[data-community-page="guaranteed"] [data-guaranteed-grid])',
      '.promo-floating-slot',
      '.promo-inline-slot'
    ];
  ...
  function mountMobileDock() {
    ...
    const items = [
      ...
      { href: '/guaranteed/', label: '보증업체', icon: '✦', accent: true },
    ...
    function syncDockViewport() {
      ...
      let bottomGap = 0;
      if (vv) bottomGap = Math.max(0, Math.round(window.innerHeight - (vv.offsetTop + vv.height)));
