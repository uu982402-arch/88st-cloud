(() => {
  ...
  function renderGuaranteedCards(providers){
    const isGuaranteedPage = document.body?.getAttribute('data-community-page') === 'guaranteed' || location.pathname.startsWith('/guaranteed/');
    $$('[data-guaranteed-grid]').forEach((grid)=>{
      ...
      grid.innerHTML = providers.map((item)=>{ const pending = !!item.pending; const codeCell = pending
      ...
      const detailSummary = pending ? '광고 준비중 · 승인 후 노출' : ((item.benefits||[]).slice(0,2).join(' · ') || '공식 주소와 가입코드 확인');
      const detailUrl = pending ? '광고 준비중' : esc(item.officialDomain);
      ...
      return `<article class="guaranteed-card guaranteed-card--panel${pending ? ' is-pending' : ''}" data-theme="${esc(item.theme)}"><div class="guaranteed-summary"><span class="guaranteed-summary-text">${esc(item.oneLine)}</span><span class="guaranteed-status">${statusLabel}</span></div><div class="guaranteed-table"><div class="guaranteed-row"><span class="guaranteed-label">이름</span><strong class="guaranteed-value">${esc(item.name)}</strong></div><div class="guaranteed-row"><span class="guaranteed-label">가입코드</span>${codeCell}</div></div><div class="guaranteed-detail" hidden><div class="guaranteed-detail-line"><span>공식주소</span><strong>${detailUrl}</strong></div><div class="guaranteed-detail-line"><span>핵심확인</span><strong>${esc(detailSummary)}</strong></div><div class="tag-row">${(item.tags||[]).slice(0,3).map((t)=>`<span class="tag-chip">${esc(t)}</span>`).join('')}</div></div><div class="guaranteed-actions"><button class="guaranteed-toggle" type="button" data-guaranteed-toggle aria-expanded="false"><span>상세보기</span></button>${actionLink}</div></article>`; }).join('');
    });
  }
