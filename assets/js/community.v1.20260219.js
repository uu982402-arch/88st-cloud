(() => {
  ...
  function getBoard(){
    const bodyBoard = String(document.body?.dataset?.board || '').trim().toLowerCase();
  ...
  function toPostUrl(id, board){
    ...
    qp.set('id', String(id));
    qp.set('board', b);
    return `/community/post/?${qp.toString()}`;
  }
  ...
  function syncBoardTabs(board){
    ...
    tabs.forEach(a => {
      const b = String(a.getAttribute('data-board-tab')||'').trim().toLowerCase();
      const on = (b === board);
  ...
  async function initIndex(){
    ...
    syncBoardTabs(board);
    const heroWrite = document.querySelector('.com-hero .st-guide-actions a.btn.primary');
    if (heroWrite) heroWrite.setAttribute('href', BOARD_UI[board].writePath);
    ...
    async function load(){
      ...
      if (!items.length){
        const msg = (board === 'promo')
          ? '아직 홍보글이 없습니다. 첫 홍보글을 작성해보세요.'
          : '아직 글이 없습니다. 첫 글을 작성해보세요.';
        listEl.innerHTML = `<div class="com-empty">${msg}</div>`;
        return;
      }
  ...
  async function initPost(){
    ...
    async function load(){
      ...
      if (data?.post?.board && VALID_BOARDS.includes(String(data.post.board).toLowerCase())) {
        board = String(data.post.board).toLowerCase();
      }

      // sync hero + links
      syncBoardTabs(board);
      $$('a[href="/community/"]').forEach(a => a.setAttribute('href', BOARD_UI[board].listPath));
      $$('a[href="/community/write/"]').forEach(a => a.setAttribute('href', BOARD_UI[board].writePath));
  ...
  async function initWrite(){
    ...
    const cancelLink = document.querySelector('#postForm .btnRow a.btn.ghost');
    if (cancelLink) cancelLink.setAttribute('href', BOARD_UI[board].listPath);
    ...
    form?.addEventListener('submit', async (e) => {
      ...
      try{
        const data = await api('/api/posts', {
          method:'POST',
          headers:{ 'content-type':'application/json' },
          body: JSON.stringify({ board, title, author_name:name, body, turnstileToken: token })
        });
        const id = data?.id;
