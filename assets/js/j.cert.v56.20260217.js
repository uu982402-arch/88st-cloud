...
(() => {
  ...
  const CARDS = {
    card1: { title: 'VEGAS', code: '6789', link: 'https://las403.com', telegram: 'UZU59', detail: '/cert/?v=card1',
      benefit: '스포츠·고액전용 입플 최대 30% 페이백 / 카지노 입플',
      notice: '가입코드 미입력 시 혜택 적용 불가' },
card3: { title: '777 Bet', code: '6767', link: 'https://82clf.com/?code=6767', telegram: 'UZU59', detail: '/cert/?v=card3',
      benefit: '가입코드 6767 전용 혜택 / 스포츠·카지노 이용 가능',
      notice: '가입코드 미입력 시 혜택 적용 불가' },
    card_jakum: { title: '자쿰', code: 'zk888', link: 'https://zk-777.com/?code=zk888', telegram: 'UZU59', detail: '/guaranteed/jakum/',
      benefit: '테더(USDT) 기반 시스템 / 신규 스포츠 첫충전 40% / 입플 5+3·10+5·50+20·100+35·200+70 / 스포츠·미니게임·슬롯 이벤트',
      notice: '가입코드 zk888 확인 후 이용하세요. 조건·롤링·이벤트 공지는 가입 전 재확인 권장' },
  };

  const TOP_IDS = ['card1','card_jakum','card3'];
...
const cardSourcesById = (id) => {
    ...
    const map = {
      ...
      card_jakum: '/assets/img/guaranteed/cards/jakum.webp?v=96',
  ...
  const shareLinkFor = (id) => {
    try {
      ...
      u.pathname = window.location.pathname.startsWith('/guaranteed') ? '/guaranteed/' : '/cert/';
      ...
      return `
        <div class="card card--imageOnly ${TOP_SET.has(id) ? 'featured' : ''}" data-card="${id}" role="button" tabindex="0"
          ...
          <div class="img-box">
                 ...
                 width="1200" height="675" fetchpriority="${TOP_SET.has(id) ? 'high' : 'auto'}"
  ...
  const openCard = (id) => {
    ...
    if (detailEl) detailEl.href = c.detail || shareLinkFor(id);
  ...
  const init = () => {
    ...
    if (detailEl) {
      try { detailEl.setAttribute('data-no-auto-outbound', '1'); detailEl.setAttribute('data-outbound-manual', '1'); } catch { /* ignore */ }
      detailEl.addEventListener('click', () => {
        try {
          trackSafe('vendor_detail_click', {
            vendor_id: currentCardId,
            vendor_name: (CARDS[currentCardId] || {}).title || '',
            url: detailEl.href,
          });
    ...
    if (linkEl) {
      ...
      linkEl.addEventListener('click', async (ev) => {
        ...
        try {
          ...
          trackSafe('vendor_outbound_click', {
            vendor_id: currentCardId,
            vendor_name: (CARDS[currentCardId] || {}).title || '',
            url: targetUrl,
          });
        } catch { /* ignore */ }
        try { window.open(targetUrl, '_blank', 'noopener,noreferrer'); } catch { window.location.href = targetUrl; }
      });
    }
