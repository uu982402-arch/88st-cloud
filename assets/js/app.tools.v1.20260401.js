...
- 추가 자료:`; const html=section('자료 체크','빠진 항목부터 봅니다.',`<div class="tool-grid tool-grid--dense">${checklist.map((item)=>`<article class="tool-help-card"><h3>${esc(item[0])}</h3><p>${item[1]?'확보':'입력 필요'}</p></article>`).join('')}</div>`)+section('전달용 요약','바로 복사할 수 있습니다.',`<div class="toolkit-copy-box"><pre>${esc(text)}</pre><div class="toolkit-actions"><button class="safety-copy-btn mint" type="button" data-inline-copy="${esc(text)}">텍스트 복사</button></div></div>`); setResult(result, html, 'toolkit-result-stack'); }); }
  ...
  function engineSportsStudy({ market='moneyline', odds=[], capital=0, mode='neutral', line=0, labels=[] }={}) {
    ...
    const secondProb = outcomes.slice().sort((a,b)=>b.prob-a.prob)[1] || null;
    const secondEdge = outcomes.slice().sort((a,b)=>b.ev.evRate-a.ev.evRate)[1] || null;
    ...
    const confidence = engines.marketConfidence ? engines.marketConfidence({ fair, outcomes, market, line, volatilityScore: score }) : { score: 50, label: '중립', action: '관망', probGap: 0, edgeGap: 0 };
    const lean = engines.marketLean ? engines.marketLean({ fair, outcomes, market, line }) : { title: '균형 시장', detail: '입력값을 더 넣으면 시장 균형을 해석합니다.' };
    return { fair, outcomes, bestProb, bestEdge, secondProb, secondEdge, volatilityScore: score, volatilityLabel: toolRiskLabel(score), bankroll, confidence, lean, line };
  }
  ...
  function initAiSportsOddsAnalysis(){
    ...
    form.addEventListener('submit',(e)=>{
      ...
      if(m==='sports'){
        const odds=[Number($('#centerOddA')?.value||0),Number($('#centerOddB')?.value||0),Number($('#centerOddC')?.value||0)].filter(Boolean); const capital=Number($('#centerCapital')?.value||0); const risk=$('#centerRisk')?.value||'neutral'; const mk=$('#centerMarket')?.value||'1x2'; if(odds.length<2) return empty(result,'입력값이 비어 있습니다.','배당 두 개 이상을 입력해 주세요.'); const labels=mk==='1x2'?['홈','무','원정']:mk==='moneyline'?['홈','원정']:mk==='ou'?['오버','언더']:['홈','원정']; const study=engineSportsStudy({ market:mk, odds, capital, mode:risk, line:Number($('#centerLine')?.value||0), labels}); const cards=[]; if(showFair) cards.push({label:'공정확률', value:`${study.bestProb.label} ${(study.bestProb.prob*100).toFixed(1)}%`, note:`북마진 ${(study.fair.margin*100).toFixed(1)}% · 신뢰도 ${Math.round(study.confidence.score)}점`}); if(showEv) cards.push({label:'기대값', value:`${(study.bestEdge.ev.evRate*100).toFixed(1)}%`, note:`${study.bestEdge.label} · ${study.confidence.action}`}); if(showRisk) cards.push({label:'변동성', value:study.volatilityLabel, note:`연패위험 ${(study.outcomes[0].vol.streakLossProb*100).toFixed(1)}% · ${study.lean.title}`}); if(showBank) cards.push({label:'추천 비중', value:study.bankroll.amount?`${Math.round(study.bankroll.amount).toLocaleString()}원`:`${(study.bankroll.ratio*100).toFixed(1)}%`, note:`${study.bankroll.label} · 손절 ${capital?Math.round(study.bankroll.stopLoss).toLocaleString()+'원':'자본 입력 시 표시'}`}); engineSet(result,'AI 스포츠 배당분석',cards,[`${study.lean.title} · ${study.lean.detail}`, `${study.bestEdge.label} 기준 보정 EV는 ${engines.formatSignedPercent(study.bestEdge.ev.evRate*100)} 입니다.`, `${study.confidence.action} 판단이며 자본 기준 손실 한도는 ${capital?Math.round(study.bankroll.stopLoss).toLocaleString()+'원':'입력 후 계산'} 입니다.`]); return;
      }
      if(m==='casino'){
...
- ${safeCode}${safeNote}`;
  ...
  function initAiReportDraft(){
    ...
    form.addEventListener('submit',(e)=>{
      ...
      const filledCount=checklist.filter((item)=>item[1]).length;
      const missing=checklist.filter((item)=>!item[1]).map((item)=>item[0]);
      const readiness = filledCount >= 5 ? '즉시 초안 가능' : (filledCount >= 3 ? '기본 초안 가능' : '정보 보강 필요');
      const focus = issue !== '-' ? issue : (story.length > 40 ? '상황 설명 중심' : '기본 정보 중심');
      const draft=`[기본 정보]
- 사이트명: ${site}
- 주소: ${domain}
- 가입코드: ${code}
...
- 주소 변경 여부`;
      const html = renderMetricRow([
        { label:'준비 상태', value: readiness, note:`입력 ${filledCount}/6` },
        { label:'누락 항목', value: missing.length ? `${missing.length}개` : '없음', note: missing.length ? missing.join(' · ') : '기본 자료 확보' },
        { label:'핵심 초점', value: focus, note:'제보 문구 우선순위' },
        { label:'복사 준비', value: '완료', note:'초안 텍스트 생성' }
      ]) + section('요약칩','빠진 자료와 현재 상태를 먼저 확인합니다.', `<div class="tool-grid tool-grid--dense">${checklist.map((item)=>`<article class="tool-help-card"><h3>${esc(item[0])}</h3><p>${item[1] ? '입력됨' : '추가 필요'}</p></article>`).join('')}</div>`) + section('핵심 결과','바로 복사할 수 있는 제보 초안을 먼저 확인합니다.', `<div class="tool-output-box"><pre>${esc(draft)}</pre></div>`) + section('세부 근거','추가로 챙기면 좋은 자료입니다.', `<ul class="toolkit-points"><li>${missing.length ? `${esc(missing.join(', '))} 항목은 먼저 보강하는 편이 좋습니다.` : '기본 자료는 비교적 채워져 있습니다.'}</li><li>입금/출금 내역은 시간 정보가 보이게 캡처해 두는 편이 좋습니다.</li><li>공지·채널·대화 내용은 같은 날짜 기준으로 함께 남기면 정리가 쉬워집니다.</li></ul>`);
      setResult(result, html, 'toolkit-result-stack');
    });
  }
