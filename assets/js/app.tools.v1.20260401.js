...
(() => {
  ...
  function formDraftKey(form){ return `raven_tool_draft_v2:${location.pathname}:${form.id || 'form'}`; }
  function persistFormDraft(form){
    if(!form || !window.localStorage) return;
    const fields = Array.from(form.querySelectorAll('input, textarea, select')).filter((field)=>field.name || field.id).filter((field)=>!['button','submit','reset','file','hidden'].includes((field.type||'').toLowerCase()));
    ...
    fields.forEach((field)=>{
      ...
      if(Object.prototype.hasOwnProperty.call(saved, fieldKey)){
        if(field.type === 'checkbox') field.checked = !!saved[fieldKey];
        else if(field.type === 'radio') field.checked = String(saved[fieldKey]) === String(field.value);
        else field.value = saved[fieldKey];
      }
    });
    const write = ()=>{
      const payload = {};
      fields.forEach((field)=>{
        const fieldKey = field.name || field.id;
        if(field.type === 'checkbox') payload[fieldKey] = !!field.checked;
        else if(field.type === 'radio') { if(field.checked) payload[fieldKey] = field.value; }
        else payload[fieldKey] = field.value;
      });
  ...
  function initResultCopyButtons(){
    document.addEventListener('click', async (e)=>{
      ...
      if(!btn) return;
      const target = $(btn.getAttribute('data-copy-result') || '');
      if(!target) return;
      const text = String(target.innerText || '').replace(/\n{3,}/g,'\n\n').trim();
      if(!text) return toast('복사할 결과가 아직 없습니다.');
...
- 대화 내용
- 주소 변경 여부`;
      const summary = renderMetricRow([
        { label:'입력 항목', value:`${filled}/6`, note:'기본 정보 충족' },
        { label:'빠진 항목', value:String(6 - filled), note:'추가 자료 확인' },
        { label:'초안 길이', value:`${draft.length}자`, note:'복사 준비' },
        { label:'판단', value: filled >= 4 ? '전달 가능' : '보강 필요', note:'핵심 정보 기준' }
      ]);
      const html = section('요약칩','먼저 보고 채우면 좋은 핵심입니다.', summary) + section('핵심 결과','빠진 항목부터 먼저 봅니다.', `<div class="tool-grid tool-grid--dense">${checklist.map((item)=>`<article class="tool-help-card"><h3>${esc(item[0])}</h3><p>${item[1] ? '입력됨' : '추가 필요'}</p></article>`).join('')}</div>`) + section('세부 근거','필요한 곳에 그대로 붙여 넣을 수 있게 정리했습니다.', `<div class="tool-output-box"><pre>${esc(draft)}</pre><div class="tool-inline-actions"><button type="button" class="safety-copy-btn mint" data-inline-copy="${esc(draft)}">초안 복사</button></div></div>`) + section('실전 메모','추가로 챙기면 좋은 자료입니다.', '<ul class="toolkit-points"><li>입금/출금 내역은 시간 정보가 보이게 캡처해 두는 편이 좋습니다.</li><li>공지·채널·대화 내용은 같은 날짜 기준으로 함께 남기면 정리가 쉬워집니다.</li></ul>');
      setResult(result, html, 'toolkit-result-stack'); }); }
  ...
  function initAiDomainAnalysis(){
    ...
    form.addEventListener('submit',(e)=>{
      ...
      const verdict = score >= 70 ? '연결 신호 높음' : score >= 40 ? '추가 확인' : '분리 가능성';
      const summary = renderMetricRow([
        { label:'기준 후보', value: official, note:'입력 기준 대표 주소' },
        { label:'주소 수', value:`${domains.length||0}개`, note: domains.length > 1 ? '후보 다수' : '후보 적음' },
        { label:'코드/채널', value:`${codes.length + handles.length}개`, note:`코드 ${codes.length||0} · 채널 ${handles.length||0}` },
        { label:'판단', value: verdict, note: `${score}점 · ${toolRiskLabel(score)}` }
      ]);
      parts.unshift(section('요약칩','먼저 보고 넘어갈 핵심 결과입니다.', summary));
      if(relation?.checked){
        ...
        parts.push(section('세부 근거','겹치는 운영 패턴을 추정으로 묶어 봅니다.',cards));
      }
      ...
      parts.push(section('핵심 결과','지금 바로 확인할 포인트만 남깁니다.',`<ul class="toolkit-points">${memo.map((m)=>`<li>${esc(m)}</li>`).join('')}</ul><div class="toolkit-actions"><a class="safety-link-btn ghost" href="/tools/ai-notice-check/">공지·후기 비교</a><a class="safety-link-btn ghost" href="/tools/ai-domain-analysis/?focus=address">공식주소 체크</a></div>`));
