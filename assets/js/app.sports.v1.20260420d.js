(() => {
  ...
  function riskProfile(mode) {
    if (mode === 'safe') return {
      label: '보수', primary: '선별 진입', secondary: '추가 확인', maxRatio: 1.2,
      evFloor: 0.012, gapFloor: 0.04, minScore: 66,
      primaryDetail: '마진 낮고 확률 격차가 선명할 때만 접근합니다.',
      secondaryDetail: '추가 비교 후 소액 접근이 더 낫습니다.',
      watchDetail: '조건이 모이면 단폴 위주로만 접근합니다.',
      splitRule: '단폴 우선 · 확인 후 진입', exposure: '최대 1.2%'
    };
    if (mode === 'aggressive') return {
      label: '공격', primary: '가치 우선', secondary: '분할 진입', maxRatio: 4.0,
      evFloor: 0.000, gapFloor: 0.015, minScore: 48,
      primaryDetail: '미세 우세도 허용하되 분할 접근을 전제로 봅니다.',
      secondaryDetail: '라인 비교 후 빠르게 진입 여부를 정합니다.',
      watchDetail: '우위가 약하면 주력 대신 분산 접근이 적합합니다.',
      splitRule: '분할 접근 · 라인 비교 필수', exposure: '최대 4.0%'
    };
    return {
      label: '중립', primary: '기본 진입', secondary: '소액 분할', maxRatio: 2.5,
      evFloor: 0.004, gapFloor: 0.025, minScore: 58,
      primaryDetail: '플러스 EV와 확률 우위를 같이 확인합니다.',
      secondaryDetail: '2순위와 비교해 소액 분할이 적합합니다.',
      watchDetail: '확률 격차가 좁으면 관망 쪽이 더 안전합니다.',
      splitRule: '단폴 + 소액 분할', exposure: '최대 2.5%'
    };
  }
  ...
  function buildSportsStudy({ engines, market, odds, riskMode, line, labels }) {
    const fair = engines.fairProbability({ odds, market });
    const outcomes = fair.fairProbabilities.map((prob, idx) => {
      const ev = engines.expectedValue({ probability: prob, odds: odds[idx], stake: 100000 });
      const vol = engines.volatilityRisk({ probability: prob, odds: odds[idx], stake: 1000, plays: market === '1x2' ? 3 : 2 });
      return {
        idx,
        label: labels[idx],
        prob,
    ...
    const bankroll = engines.bankrollPlan({ capital: 0, probability: bestEdge.prob, odds: bestEdge.odds, mode: riskMode, volatilityScore: marketVol.score });
    ...
    const risk = riskProfile(riskMode);

    let riskCall = { title: '관망', detail: risk.watchDetail, tone: 'low' };
    const margin = fair.margin || 0;
    const evRate = bestEdge?.ev?.evRate || 0;
    const probGap = confidence.probGap || 0;
    const score = confidence.score || 0;
    if (margin < 0.09 && score >= risk.minScore && evRate >= risk.evFloor && probGap >= risk.gapFloor) {
      riskCall = { title: risk.primary, detail: risk.primaryDetail, tone: 'high' };
    } else if (margin < 0.1 && score >= (risk.minScore - 8) && evRate > Math.max(-0.002, risk.evFloor - 0.01)) {
      riskCall = { title: risk.secondary, detail: risk.secondaryDetail, tone: 'mid' };
    }

    return { fair, outcomes, bestProb, bestEdge, marketVol, bankroll, confidence, lean, line, risk, riskCall };
  }

  function sportsComment({ market, study, line, engines }) {
    const comments = [];
    ...
    comments.push(study.fair.margin >= 0.07 ? `북마진 ${cleanPercent(study.fair.margin * 100)}로 높아 다른 한 곳 더 비교하는 편이 좋습니다.` : `북마진 ${cleanPercent(study.fair.margin * 100)}로 과도하지는 않습니다.`);
    if (market === 'ou' && line) comments.push(`기준점 ${line} 전후 0.5 차이도 같이 확인하세요.`);
    if (market === 'hcp' && line) comments.push(`핸디 ${line}는 푸시·커버 시나리오를 같이 봐야 합니다.`);
    comments.push(`${riskText(study.bankroll.mode)} 운용 기준 ${study.riskCall.title} · ${study.risk.exposure}`);
  ...
  function buildPlanNotes({ market, study, line, engines }) {
    const notes = [];
    notes.push(`${study.risk.label} 운용 · ${study.riskCall.title}`);
    notes.push(`권장 비중 ${(study.bankroll.ratio * 100).toFixed(1)}% · ${study.risk.exposure}`);
    notes.push(study.fair.margin >= 0.07 ? '다른 북 한 군데 더 비교' : '현재 마진은 과도하지 않음');
    if (market === 'ou' && line) notes.push(`라인 ${line} 전후 0.5 차이도 같이 보기`);
    else if (market === 'hcp' && line) notes.push(`핸디 ${line} 푸시·커버 시나리오 체크`);
    else notes.push('단일 선택보다 2순위와 격차 같이 보기');
    if ((study.bestEdge?.ev?.evRate || 0) > 0) notes.push(`${study.bestEdge.label} ${engines.formatSignedPercent(study.bestEdge.ev.evRate * 100)} 구간`);
    return notes.slice(0, 4);
  }

  function renderLabIdle(lab, label) {
    if (!lab) return;
    lab.innerHTML = `
      <div class="sports-lab-shell is-idle">
        <div class="sports-lab-head"><strong>추가 분석</strong><span>${esc(label)} 대기</span></div>
        <div class="sports-lab-grid">
          <article class="sports-lab-card"><span>배당 구조</span><strong>-</strong><small>마진 · 공정오즈</small></article>
          <article class="sports-lab-card"><span>확률 격차</span><strong>-</strong><small>1순위 vs 2순위</small></article>
          <article class="sports-lab-card"><span>운용 성향</span><strong>-</strong><small>보수 · 중립 · 공격</small></article>
          <article class="sports-lab-card"><span>라인 메모</span><strong>-</strong><small>시장별 체크</small></article>
        </div>
        <ul class="sports-lab-notes"><li>배당을 넣으면 추가 분석이 자동으로 채워집니다.</li></ul>
      </div>`;
  }

  function renderLabPanel({ lab, market, study, line, engines }) {
    if (!lab || !study) return;
    const secondProb = outcomeSort(study.outcomes, 'prob')[1];
    const gapText = secondProb ? `${(study.confidence.probGap * 100).toFixed(1)}%p` : '단일 값';
    const lineText = market === 'ou' || market === 'hcp'
      ? (line ? `${line} 기준 체크` : '기준점 대기')
      : (study.lean.title.includes('무승부') ? '무승부 경계' : '1순위 흐름 우선');
    lab.innerHTML = `
      <div class="sports-lab-shell">
        <div class="sports-lab-head"><strong>추가 분석</strong><span>${esc(study.risk.label)} · ${esc(study.riskCall.title)}</span></div>
        <div class="sports-lab-grid">
          <article class="sports-lab-card"><span>배당 구조</span><strong>마진 ${cleanPercent(study.fair.margin * 100)}</strong><small>공정오즈 ${study.bestProb.fairOdds.toFixed(2)}</small></article>
          <article class="sports-lab-card"><span>확률 격차</span><strong>${esc(gapText)}</strong><small>${esc(study.bestProb.label)} 우세 기준</small></article>
          <article class="sports-lab-card"><span>운용 성향</span><strong>${esc(study.risk.exposure)}</strong><small>${esc(study.risk.splitRule)}</small></article>
          <article class="sports-lab-card"><span>라인 메모</span><strong>${esc(lineText)}</strong><small>${esc(study.lean.title)}</small></article>
        </div>
        <ul class="sports-lab-notes">
          <li>${esc(study.riskCall.detail)}</li>
          <li>${esc(study.bestEdge.label)} 보정 EV ${engines.formatSignedPercent(study.bestEdge.ev.evRate * 100)} · ${esc(study.marketVol.grade)}</li>
          <li>${esc((market === '1x2' || market === 'moneyline') ? '최종 선택 전 2순위와 격차를 같이 보세요.' : '라인 변동과 기준점 움직임을 함께 확인하세요.')}</li>
        </ul>
      </div>`;
  }
    ...
    assist.innerHTML = `
      <div class="sports-support-shell is-idle">
        ...
        <div class="sports-support-grid">
          ...
          <article class="sports-support-card"><span>운영</span><strong>-</strong><small>성향별 차등</small></article>
  ...
  function renderAssistPanel({ assist, market, study, engines, line }) {
    if (!assist || !study) return;
    const signals = buildAvoidSignals({ market, study, line });
    const notes = buildPlanNotes({ market, study, line, engines });
    ...
    assist.innerHTML = `
      <div class="sports-support-shell">
        <div class="sports-support-head">
          ...
          <span class="sports-support-tone">${esc(study.riskCall.title)}</span>
        </div>
        <div class="sports-support-grid">
          ...
          <article class="sports-support-card"><span>운영</span><strong>${esc(study.risk.exposure)}</strong><small>${esc(study.risk.splitRule)}</small></article>
        ...
        <div class="sports-support-block-grid">
          ...
          <section class="sports-support-block">
            <div class="sports-support-block-head"><strong>운영 메모</strong><span>${esc(study.risk.label)}</span></div>
  ...
  function buildCompactResultHtml({ study, market, engines, line }) {
    const marketLabel = MARKETS[market]?.label || '스포츠';
    const notes = sportsComment({ market, study, line, engines }).slice(0, 3);
    ...
    return `
      <div class="sports-score-shell sports-score-shell--modal">
        ...
        <div class="sports-summary-card">
          ...
          <p>${esc(`${study.riskCall.title} · ${study.bestEdge.label} 보정 EV ${engines.formatSignedPercent(study.bestEdge.ev.evRate * 100)}`)}</p>
        </div>
        <div class="sports-insight-grid sports-insight-grid--compact">
          ...
          <article class="sports-insight-card"><span>운용</span><strong>${esc(study.risk.exposure)}</strong><small>${esc(`${study.risk.label} · ${study.riskCall.title}`)}</small></article>
  ...
  function buildModalHtml({ market, study, engines, line, assistHtml }) {
    const title = `${MARKETS[market]?.label || '스포츠'} 분석 결과`;
    const mainHtml = buildCompactResultHtml({ study, market, engines, line });
    return {
      title,
      html: `
        <div class="sports-modal-stack">
          <div class="sports-modal-callout"><strong>${esc(study.riskCall.title)}</strong><span>${esc(study.risk.label)} · 신뢰도 ${Math.round(study.confidence.score)}점 · ${esc(study.lean.title)}</span></div>
  ...
  function initSports() {
    ...
    const lab = qs('[data-sports-mini-lab]');
    const tabWrap = qs('[data-sports-tabs]');
    if (!form || !result || !tabWrap || !assist || !lab || !engines.fairProbability) return;
    ...
    const inputs = Object.fromEntries(['line', 'home', 'draw', 'away', 'over', 'under', 'risk'].map((k) => [k, qs(`[data-field="${k}"]`, form)]));
    ...
    function setIdle(label) {
      ...
      if (formNote) formNote.textContent = '시장별 체크 포인트와 운용 메모를 같이 봅니다.';
    ...
    function compute() {
      ...
      const study = buildSportsStudy({ engines, market, odds, riskMode, line: cleanLine, labels: cfg.resultLabels });
      lastStudy = study;
      hasValidResult = true;
      ...
      if (subsummary) subsummary.textContent = `${study.riskCall.title} · ${study.bestEdge.label} 보정 EV ${engines.formatSignedPercent(study.bestEdge.ev.evRate * 100)}`;
      if (marginNode) marginNode.textContent = `북마진 ${(study.fair.margin * 100).toFixed(1)}%`;
      if (formNote) formNote.textContent = `${study.lean.title} · ${study.riskCall.detail}`;
      ...
      if (insightValue('bank')) insightValue('bank').textContent = `${(study.bankroll.ratio * 100).toFixed(1)}%`;
      if (insightNote('bank')) insightNote('bank').textContent = `${study.risk.label} · ${study.risk.exposure}`;
      ...
      if (insightValue('call')) insightValue('call').textContent = study.riskCall.title;
      if (insightNote('call')) insightNote('call').textContent = study.risk.label;
      if (notesNode) {
        const notes = sportsComment({ market, study, line: cleanLine !== 0 ? cleanLine : '', engines });
        notesNode.innerHTML = notes.map((item) => `<li>${esc(item)}</li>`).join('');
      }
      renderAssistPanel({ assist, market, study, engines, line: cleanLine !== 0 ? cleanLine : '' });
      renderLabPanel({ lab, market, study, line: cleanLine !== 0 ? cleanLine : '', engines });
    }
    ...
    form.addEventListener('submit', (e) => {
      ...
      const payload = buildModalHtml({
        market: hiddenMarket.value || '1x2',
        study: lastStudy,
        engines,
        line: Number.isFinite(Number(inputs.line?.value || '')) && Number(inputs.line?.value || '') !== 0 ? Number(inputs.line?.value || '') : '',
        assistHtml: assist.innerHTML,
      });
      modal()?.open({ titleText: payload.title, html: payload.html });
    });
