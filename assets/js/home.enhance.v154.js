...
(function(){
  ...
  function injectTodayStatus(cfg){
    ...
    sec.innerHTML = [
      ...
      '<div class="dash-sub">세이프티 락과 오늘 손익을 한 화면에서 확인합니다. <span class="muted">(로컬 저장)</span></div>',
      '<div class="homeStatusRow">',
      '  <div class="homeStatusPill"><div><div class="l">세이프티</div><div class="v" id="_hsLock">'+escapeHtml((locked?'LOCK':(on?'ON':'OFF')) )+'</div></div></div>',
  ...
  function bindQuickLog(cfg){
    ...
    function open(){
      var day = todayKey();
      openModal('빠른 기록', [
        '<div class="p">간단히 기록만 남기세요. (나중에 로그북에서 상세 수정 가능)</div>',
        '<div class="row">',
        '  <select id="_qlCat">',
        ...
        '  <button class="btn" type="button" id="_qlOpen" style="flex:1">로그북 열기</button>',
        '</div>'
      ].join(''));
      ...
      if(save) save.addEventListener('click', function(){
        ...
        // record usage
        try{ if(window.__88st_recent && window.__88st_recent.record) window.__88st_recent.record('/logbook/', '베팅 로그북', {cat:'log', type:'log'}); }catch(e){}
