...
(() => {
  ...
  function renderHomeAiPlaceholder(target){
    if(!target) return;
    target.innerHTML = `<section class="desktop-ai-panel" aria-label="AI 조회 결과"><div class="desktop-ai-shell desktop-ai-shell--placeholder"><div class="desktop-ai-head"><div><span class="desktop-ai-kicker">AI 조회</span><strong>사이트명 또는 도메인을 넣고 바로 확인하세요.</strong></div><span class="mini-badge">핵심 4항목</span></div><div class="desktop-ai-facts"><article class="desktop-ai-fact"><span>등록일</span><strong>입력 후 확인</strong></article><article class="desktop-ai-fact"><span>최종 연결</span><strong>입력 후 확인</strong></article><article class="desktop-ai-fact"><span>페이지 제목</span><strong>입력 후 확인</strong></article></div><div class="desktop-ai-grid desktop-ai-grid--compact">${buildAiCard('주소 변경 이력','입력 후 확인','확인된 도메인 흐름만 짧게 요약합니다.')}${buildAiCard('리뉴얼 흔적','입력 후 확인','이전 운영명·문구 유사도를 먼저 봅니다.')}${buildAiCard('계열사 유사도','입력 후 확인','확정이 아닌 유사 신호 기준으로만 표시합니다.')}${buildAiCard('다음 확인','입력 후 안내','공식주소 체크와 보증업체 대조 순서만 보여줍니다.')}</div></div></section>`;
  }
  ...
  function renderHomeAiResult(target, payload){
    ...
    target.innerHTML = `<section class="desktop-ai-panel" aria-label="AI 조회 결과"><div class="desktop-ai-shell" data-tone="${esc(verdict.tone || 'neutral')}"><div class="desktop-ai-head"><div><span class="desktop-ai-kicker">AI 조회 결과</span><strong>${esc(verdict.label || '추가 확인 필요')}</strong></div><div class="desktop-ai-head-actions"><span class="desktop-ai-tone">${esc(aiToneLabel(verdict.tone || 'neutral'))}</span><a class="safety-link-btn ghost" href="${googleHref}" target="_blank" rel="noopener noreferrer">원문 검색</a></div></div>${sourceChips.length ? `<div class="desktop-ai-inline-list">${sourceChips.map((chip)=>`<span>${esc(chip)}</span>`).join('')}</div>` : ''}<div class="desktop-ai-facts">${factItems}</div><div class="desktop-ai-grid">${buildAiCard('주소 변경 이력', cards.history?.value || '-', cards.history?.body || '-')}${buildAiCard('리뉴얼 흔적', cards.renewal?.value || '-', cards.renewal?.body || '-')}${buildAiCard('계열사 유사도', cards.affinity?.value || '-', cards.affinity?.body || '-')}<article class="desktop-ai-card desktop-ai-card--list"><span>다음 확인</span><strong>${esc(cautions.length ? `${cautions.length}개` : '기본 체크')}</strong><ul>${(cautions.length ? cautions : ['검색 결과와 공지 채널을 같이 보세요.']).slice(0,3).map((item)=>`<li>${esc(item)}</li>`).join('')}</ul>${nextSteps.length ? `<div class="desktop-ai-inline-list">${nextSteps.slice(0,3).map((step)=>`<span>${esc(step.label || '다음 단계')}</span>`).join('')}</div>` : ''}</article></div><div class="desktop-ai-summary">${esc(verdict.summary || '입력값 기준으로 먼저 볼 항목만 짧게 정리했습니다.')}</div></div></section>`;
  }
  function wireHomeAiLookup(){
    ...
    form.addEventListener('submit', async (e)=>{
      ...
      if(target){
        target.innerHTML = `<section class="desktop-ai-panel" aria-label="AI 조회 결과"><div class="desktop-ai-shell"><div class="desktop-ai-head"><div><span class="desktop-ai-kicker">AI 조회 결과</span><strong>조회 중입니다.</strong></div><span class="mini-badge">${esc(clean)}</span></div><div class="desktop-ai-facts">${buildAiFact('등록일','확인 중')}${buildAiFact('최종 연결','확인 중')}${buildAiFact('페이지 제목','확인 중')}</div></div></section>`;
      }
      try{
