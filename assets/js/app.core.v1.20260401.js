...
(() => {
  ...
  function providerCard(item){
    ...
    return `<article class="guaranteed-card" data-theme="${esc(item.theme)}"><div class="guaranteed-summary"><span class="guaranteed-summary-text">${esc(item.oneLine || '공식 주소와 가입코드 확인')}</span><span class="guaranteed-status">${pending ? '준비중' : '운영중'}</span></div><div class="guaranteed-table"><div class="guaranteed-row"><span class="guaranteed-label">이름</span><strong class="guaranteed-value">${esc(item.name)}</strong></div><div class="guaranteed-row"><span class="guaranteed-label guaranteed-label--code">가입코드</span>${code}</div></div><div class="guaranteed-detail" hidden><div class="guaranteed-detail-grid"><div class="guaranteed-detail-line"><span>공식주소</span><strong>${esc(item.officialDomain || item.officialUrl || '-')}</strong></div><div class="guaranteed-detail-line"><span>확인기준</span><strong>${esc((item.benefits || []).slice(0,2).join(' · ') || '공식 주소와 코드 확인')}</strong></div><div class="guaranteed-detail-line"><span>태그</span><strong>${esc((item.tags || []).slice(0,3).join(' · ') || '-')}</strong></div><div class="guaranteed-detail-line"><span>상태</span><strong>${pending ? '광고 준비중' : '운영중'}</strong></div></div></div><div class="guaranteed-actions"><button class="guaranteed-toggle" type="button" data-guaranteed-toggle aria-expanded="false"><span>상세보기</span></button>${action}</div></article>`;
  }
  ...
  function wireGuaranteed(){
    document.addEventListener('click', async (e)=>{
      ...
      if (open) { detail.removeAttribute('hidden'); toggle.setAttribute('aria-expanded','true'); toggle.querySelector('span').textContent='접기'; }
      else { detail.setAttribute('hidden',''); toggle.setAttribute('aria-expanded','false'); toggle.querySelector('span').textContent='상세보기'; }
    });
  }
