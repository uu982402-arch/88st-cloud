(() => {
  ...
  const page = isHome ? 'home' : isGuaranteed ? 'guaranteed' : isTools ? 'tools' : 'blog';
  ...
  function createModal(items) {
    ...
    root.hidden = true;
    const title = isSoft ? '보증업체 빠른 확인' : '오늘 확인할 공식 주소';
    ...
    root.addEventListener('click', async (event) => {
      ...
      const linkTrigger = event.target.closest('[data-hub-copy-link]');
      if (linkTrigger) {
        copyText(linkTrigger.getAttribute('data-hub-copy-link') || '');
      }
    });
