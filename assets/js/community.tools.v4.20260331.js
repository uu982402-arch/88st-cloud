(() => {
  ...
  function renderHomePreview(keyword, suffix){
    ...
    target.innerHTML = `<div class="glass-card helper-box"><div class="section-head"><div><h2>다음 단계</h2><p>바로 이어서 확인합니다.</p></div></div><div class="lookup-links"><article class="lookup-link-card"><a href="${googleUrl(q)}" target="_blank" rel="noopener noreferrer">구글에서 보기 ↗</a><p>${esc(q)}</p></article><article class="lookup-link-card"><a href="/muktu-police/search/?q=${encodeURIComponent(clean)}&type=${encodeURIComponent(suffix||'먹튀')}">구글링 페이지 ↗</a><p>검색 조합으로 이어집니다.</p></article><article class="lookup-link-card"><a href="/muktu-police/check/${domain && domain.includes('.') ? `?domain=${encodeURIComponent(domain)}`:''}">도메인·IP 조회 ↗</a><p>기술 조회로 이어집니다.</p></article></div></div>`;
  }
