(() => {
  ...
  function renderPublicEvidenceGrid(){
    $$('[data-public-evidence-grid]').forEach((grid)=>{
      grid.innerHTML = `
      <article class="community-source-card"><h3>가져올 수 있는 항목</h3><p>제목, 날짜, URL, 카테고리/태그, 본문에 언급된 도메인·IP 같은 공개 메타 항목을 추출 대상으로 둡니다.</p></article>
      <article class="community-source-card"><h3>기록 원칙</h3><p>본문 전체를 복사하지 않고, 공개 근거만 메타데이터로 남기고 원문 링크를 함께 보관하는 쪽으로 설계했습니다.</p></article>
      <article class="community-source-card"><h3>다음 단계</h3><p>검색 결과와 공개 글 근거만으로 부족하면 도메인·IP 조회로 넘어가 점수와 해석을 같이 봅니다.</p><strong>/tools 의 공개 근거 추출기 사용</strong></article>`;
    });
  }
  ...
  function renderHomePreview(keyword, suffix){
    ...
    target.innerHTML = `<div class="glass-card helper-box"><div class="section-head"><div><h2>빠른 실행</h2><p>입력한 키워드로 검색, 조회, 근거 정리를 바로 이어갈 수 있습니다.</p></div></div><div class="lookup-links"><article class="lookup-link-card"><a href="${googleUrl(q)}" target="_blank" rel="noopener noreferrer">구글에서 보기 ↗</a><p>${esc(q)}</p></article><article class="lookup-link-card"><a href="/muktu-police/search/?q=${encodeURIComponent(clean)}&type=${encodeURIComponent(suffix||'먹튀')}">구글링 페이지 ↗</a><p>검색 조합과 공개 근거 확인으로 이어서 봅니다.</p></article><article class="lookup-link-card"><a href="/muktu-police/check/${domain && domain.includes('.') ? `?domain=${encodeURIComponent(domain)}`:''}">도메인·IP 조회 ↗</a><p>기술적 기본값과 점수를 확인합니다.</p></article></div></div>`;
  }
  ...
  function renderDomainLookup(payload){
    ...
    result.innerHTML = `
      ...
      <div class="glass-card helper-box"><div class="section-head"><div><h2>기술 해석 레이어</h2><p>값을 그대로 보여주는 대신, 지금 확인한 신호를 문장으로 같이 읽습니다.</p></div></div><div class="interpretation-grid">${interp.map((item)=>`<article class="interpret-card"><span class="risk-chip">${esc(item.level || '기본')}</span><h3>${esc(item.title)}</h3><p>${esc(item.detail)}</p></article>`).join('')}</div></div>
  ...
  function renderIpLookup(payload){
    ...
    result.innerHTML = `
      ...
      <div class="glass-card helper-box"><div class="section-head"><div><h2>해석 레이어</h2><p>IP 기준으로 어떤 점을 같이 봐야 하는지 문장으로 정리했습니다.</p></div></div><div class="interpretation-grid">${interp.map((item)=>`<article class="interpret-card"><span class="risk-chip">${esc(item.level || '기본')}</span><h3>${esc(item.title)}</h3><p>${esc(item.detail)}</p></article>`).join('')}</div></div>
      <div class="glass-card helper-box"><div class="section-head"><div><h2>PTR / 네트워크 힌트</h2><p>PTR 응답과 네트워크 기본 정보를 같이 봅니다.</p></div></div><div class="lookup-links"><article class="lookup-link-card"><a href="#">PTR</a><p>${esc(ptr.join(', ') || '-')}</p></article><article class="lookup-link-card"><a href="#">도메인</a><p>${esc(info.network || '-')}</p></article><article class="lookup-link-card"><a href="#">타입</a><p>${esc(info.type || '-')}</p></article></div></div>
      <div class="glass-card helper-box"><div class="section-head"><div><h2>최근 조회 이력과 비교</h2><p>같은 ASN이나 서브넷이 최근 이력과 겹치는지 확인합니다.</p></div></div><div class="evidence-grid">${renderHistoryMatches(historyMatches)}</div></div>
